...
Last Updated Nov 24.  Enjoy.

```dataviewjs
dv.view('toc')
```
[//]: # (To update the manual contents table between the '--start/endContents' markers, copy the autogenerated toc within Obsidian and paste the updated text within the markers. Then select the text and use the following global-line substitutions to update all the links in the toc to be GitHub compatible: 
'<,'>s/%20/-/g
...
1. [Why Software Dev Guidebook](#Why-Software-Dev-Guidebook)
    ...
    4j [Review each others code and be supportive](#Review-each-others-code-and-be-supportive)
...
2. [Coding Recommendations and Best Practices](#Coding-Recommendations-and-Best-Practices)
    ...
    13. [Dependency Injection and Inversion of Control to Decrease Code Coupling - IoC](#Dependency-Injection-and-Inversion-of-Control-to-Decrease-Code-Coupling--IoC)
    14. [Avoid Paying too much Inheritance Tax - Consider Interfaces or Traits with Composition to achieve Ad-hoc Polymorphism](#Avoid-Paying-too-much-Inheritance-Tax---Consider-Interfaces-or-Traits-with-Composition-to-achieve-Ad-hoc-Polymorphism)
    ...
    15. [It Should Not be Possible to Create an Object in an Invalid State](#It-Should-Not-be-Possible-to-Create-an-Object-in-an-Invalid-State)
    16. [Know Some Design Patterns](#Know-Some-Design-Patterns)
        1. [The Strategy Pattern Example](#The-Strategy-Pattern-Example)
        2. [The Visitor Pattern](#The-Visitor-Pattern)
    17. [Consider using the Builder Pattern for More Complex Object Creation Scenarios](#Consider-using-the-Builder-Pattern-for-More-Complex-Object-Creation-Scenarios)
    18. [Information Hiding](#Information-Hiding)
    19. [Keep it Simple Stupid KISS](#Keep-it-Simple-Stupid-KISS)
3. [DRY Do not Repeat Yourself](#DRY-Do-not-Repeat-Yourself)
    1. [YAGNI You Are not Going to Need It](#YAGNI-You-Are-not-Going-to-Need-It)
    2. [Comment in line As You Go](#Comment-in-line-As-You-Go)
    3. [The Boy Scout Rule](#The-Boy-Scout-Rule)
    4. [Law of Demiter and Train Wrecks](#Law-of-Demiter-and-Train-Wrecks)
    5. [Functional vs OOP - Choose Two](#Functional-vs-OOP---Choose-Two)
    6. [Do not pollute Functional Code with Mutable State](#Do-not-pollute-Functional-Code-with-Mutable-State)
    7. [Make Private your Default Class Level Visibility](#Make-Private-your-Default-Class-Level-Visibility)
    8. [Make Immutability your Default](#Make-Immutability-your-Default)
    9. [Interior Mutability](#Interior-Mutability)
    10. [Use Calculations Where Possible to Limit Side Effects](#Use-Calculations-Where-Possible-to-Limit-Side-Effects)
    11. [Separate Operations from Calculations](#Separate-Operations-from-Calculations)
    12. [Error Handling - 4 Types of Problems](#Error-Handling---4-Types-of-Problems)
    13. [Error Handling - Exceptions vs Errors-as-Values](#Error-Handling---Exceptions-vs-Errors-as-Values)
    ...
    15. [Error Handling - Exceptions should not be used for flow control - exceptional does not mean conditional](#Error-Handling---Exceptions-should-not-be-used-for-flow-control---exceptional-does-not-mean-conditional)
    16. [Error Handling - Only use exceptions for exceptional situations such as coding errors and unexpected errors - exceptional does not mean conditional](#Error-Handling---Only-use-exceptions-for-exceptional-situations-such-as-coding-errors-and-unexpected-errors---exceptional-does-not-mean-conditional)
    17. [Error Handling - Provide relevant exceptions for the abstraction layer](#Error-Handling---Provide-relevant-exceptions-for-the-abstraction-layer)
    18. [Error Handling - Bubble exceptions upwards or trap at source](#Error-Handling---Bubble-exceptions-upwards-or-trap-at-source)
    19. [Error Handling – Model the absence of value explicitly](#Error-Handling-%E2%80%93-Model-the-absence-of-value-explicitly)
    20. [Error Handling in Functional Programming – error monads such as Either and Validated](#Error-Handling-in-Functional-Programming-%E2%80%93-error-monads-such-as-Either-and-Validated)
    21. [Data Orientated Programming with Algebraic Data Types - ADTs](#Data-Orientated-Programming-with-Algebraic-Data-Types---ADTs)
    ...
    23. [Security Development Practices](#Security-Development-Practices)
...
- Use intention revealing names for your abstractions, classes, functions & variables, and don’t use single character variable names (single chars for implicit loops etc are OK, but for global/module/class members, please use sensible names).  For example, `process_model` is too generic, what does it mean? `execute_nlp_training_model` is better, its more self-documenting.
...
c)     **To encourage good design.**  It _really_ does actually - there’s lots of supporting research that shows this. When you write tests, you put yourself in the caller perspective, so you really do think about design such as loose coupling, separation of concerns, modularity, simplicity of API, and so on. If your code is hard to test, it’s likely too strongly coupled, and here Dependency Injection with abstractions can help you.  Test Driven Development (TDD) is the ultimate testing practice, where you write tests first using an imaginary API, and then you fill in the details.
...
### Dependency Injection and Inversion of Control to Decrease Code Coupling - IoC

Basically, this means that your application code does not itself create instances of classes (dependencies), instead these objects are created separately by logic that sits outside of your immediate domain logic and passed into your domain logic via class constructors or setter methods. This ‘wiring together’ of dependencies can be done manually using the Factory Pattern or using an Inversion of Control container (I’m a fan of [Spring.io](https://stfc365.sharepoint.com/sites/HartreeRSEPublic/Shared%20Documents/General/spring.io)). For example, if classA depends on classB, the IoC container will manage the lifecycle of classB for you (i.e., creation/cacheing/destruction) and will inject classB into classA when needed. For required dependencies, meaning that classA can’t be initially created without a valid instance of classB, use ‘Constructor Injection’.  For optional dependencies, use setter injection.  The lifetime of a class is usually either singleton-scoped a.k.a., ‘application scoped’, where a single instance is created and managed (which means it must be thread-safe), or ‘prototype-scoped,’ where a new instance is always created & injected. There are several other special lifetimes such as ‘session-scope’ or 'transaction-scope' depending on your requirement.

So, what’s the benefit?  DI is likely overkill for small projects and scripts, but for large complex projects, it really is an excellent approach for decreasing _code coupling_ using interface implementations and abstractions. It also enables far more effective testing - you can mock different dependencies/classes and inject them into your objects as needed to test different scenarios. Here’s a very simple example from Dave Farley’s ‘Modern Software Engineering’ book:
...
*Dynamic Binding vs Static Binding for DI*:  Did you know that DI/IoC containers are changing a lot recently. In the past, they have largely been built using a technique known as ‘reflection’, but they are now being extended so you can choose the dependency injection method. For example, choose reflection for highly flexible Just-in-Time (JIT) dynamic binding, or static injection, which plays far nicer for scenarios that require more performant Ahead of Time (AOT) compilation at the expense of dynamism.  This is a big topic, but as ever, there are trade-offs between the different approaches which ultimately depend on your use-case. If I were building a business application that needed to be easily extended at runtime with new logic for example (e.g. using dynamic proxies and Service Provider Interfaces), then classic reflection is more suitable. If you do not need to modify runtime behaviour, choose static injection for better performance.
...
There might be a tried & tested design pattern for the problem you’re tackling. Some patterns are probably overkill, but some genuinely useful patterns include Factory, DTO, Observer, Strategy, Singleton, Repository, Stateless Façade, Visitor. Have a look at the recommended texts in the appendix. GoF is kinda regarded as old school these days.
...
A big topic, it can’t all be covered here, this ranges from single host shared memory parallelism, to multi-node clustering (e.g., HPC), to truly distributed multi-host cluster networks (e.g., remote Actors / FaaS). In computing, concurrency is not parallelism, despite the two terms having very similar dictionary definitions. Concurrency is a software concern involving context switching of a process on a CPU core via a ‘kernel thread’ (these types of thread are also commonly referred to as process thread, carrier thread, platform thread) . Context switching gives the illusion that multiple things are happening at once as the time slices are so small. True parallelism is both a software and hardware concern which requires hardware to enable doing multiple things at once. This can range from multiple cores on one CPU, multiple CPUs, nodes, local and remote actors, remote VMs, cloud functions such as Lambda and more. Here are some general recommendations:
